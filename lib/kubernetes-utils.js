...
function substituteImageNameInSpecContent(currentString, imageName, imageNameWithNewTag) {
    ...
    return currentString.split('\n').reduce((acc, line) => {
        ...
        if (imageKeyword) {
            ...
            core.debug('================> 1 >'  + line.substring(imageKeyword[0].length).trim() + '<' );
            core.debug('================> 2 >'  + line.substring(imageKeyword[0].length).trim().replace(/[',"]/g, '') + '<' );
            ...
            const lastTagSplitIndex = currentImageName.indexOf(':');
...
function updateContainerImagesInManifestFiles(contents, containers) {
    ...
    if (!!containers && containers.length > 0) {
        containers.forEach((container) => {
            ...
            const lastTagSplitIndex = imageName.indexOf(':');
            if (lastTagSplitIndex > registrySplitIndex) {
